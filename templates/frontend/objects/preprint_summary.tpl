...
<div class="obj_article_summary">
	...
	<div class="title">
		<a id="preprint-{$preprint->getId()}" {if $journal}href="{url journal=$journal->getPath() page="preprint" op="view" path=$preprintPath}"{else}href="{url page="preprint" op="view" path=$preprintPath}"{/if}>
			{$preprint->getCurrentPublication()->getLocalizedFullTitle()|strip_unsafe_html}
			{if $preprint->getCurrentPublication()->getLocalizedSubtitle()}
				<span class="subtitle">
	...
	<div class="meta">
		...
		{if $preprint->getCurrentPublication()->getData('datePublished')}
			<div class="published">
				{translate key="submission.dates" submitted=$preprint->getCurrentPublication()->getData('dateSubmitted')|date_format:$dateFormatShort published=$preprint->getCurrentPublication()->getData('datePublished')|date_format:$dateFormatShort}
			</div>
		{/if}
